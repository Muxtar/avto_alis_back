...
// Admin `otp_real` flag-ı aktivdirsə kod SMS ilə (1sms.az) göndərilir və cavabda
...
import { send1smsOtp } from './sms1az';
...
export interface OtpResult {
  code: string;
  delivered: boolean; // SMS ilə real göndərildi?
...
export async function createOtp(userId: number): Promise<OtpResult> {
  ...
  if (realMode) {
    ...
    if (u?.phone) {
      // Kodu SMS ilə göndər (1sms.az).
      delivered = (await send1smsOtp(u.phone, code)).delivered;
    }
  }

  // Kodu cavabda göstər (input üstündə "fake"):
  //  • test (fake) rejimində HƏMİŞƏ göstərilir,
  //  • real rejimdə GÖSTƏRİLMİR — kod yalnız SMS ilə gedir (göndərmə alınmasa
