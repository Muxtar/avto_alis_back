...
/** Təklif qəbul olundu — alış pəncərəsi açılır, alıcıya link. */
...
export async function expireOffers(): Promise<number> {
  ...
  for (const o of stale) {
    ...
    const msg = o.status === 'ACCEPTED' ? 'Razılaşdırılmış qiymətlə alış müddəti bitdi.' : o.status === 'COUNTERED' ? 'Əks-təklifə vaxtında cavab verilmədi.' : 'Satıcı təklifə vaxtında cavab vermədi.';
